@system VaporPressure begin
    ...
    ambient(es; T, RH): ea => es(T=T) * RH ~ call(u"kPa")
    deficit(es; T, RH): D => es(T=T) * (1 - RH) ~ call(u"kPa")
    relative_humidity(es; T, VPD): rh => 1 - VPD / es(T=T) ~ call(u"NoUnits")

    # slope of the sat vapor pressure curve: first order derivative of Es with respect to T
    saturation_slope_delta(es, b, c; T): Delta => (@nounit T; es(T=T) * (b*c)/(c+T)^2 / u"K") ~ call(u"kPa/K")
    saturation_slope(Delta; T, P): s => Delta(T=T) / P ~ call(u"K^-1")
end
