@system Area begin
    ...
    leaf_area(x=leaves.area) => sum(x) ~ track(u"cm^2")

    green_leaf_area(x=leaves.green_area) => sum(x) ~ track(u"cm^2")
    ...
    senescent_leaf_area(x=leaves.senescent_area) => sum(x) ~ track(u"cm^2")

    potential_leaf_area(x=leaves.potential_area) => sum(x) ~ track(u"cm^2")

    potential_leaf_area_increase(x=leaves.potential_area_increase) => sum(x) ~ track(u"cm^2")

    # calculate relative area increases for leaves now that they are updated
    #TODO remove if unnecessary
    # relative_leaf_area_increase(x=leaves.releative_area_increase) => sum(x) ~ track(u"cm^2")
