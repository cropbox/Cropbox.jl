...
Base.show(io::IO, ::MIME"text/html", c::Config) = begin
    ...
    f((s, C)) = begin
        ...
        println(b, "<td colspan=\"3\" style=\"text-align: left; padding-left: 20px; color: rebeccapurple\">$s</th>")
        println(b, "</tr>")
        K = keys(C)
        l = isempty(K) ? 0 : maximum(length.(string.(K)))
        for (k, v) in C
            print(b, "<tr style=\"background-color: transparent\">")
            print(b, "<td style=\"text-align: left; padding-left: 40px; color: royalblue\">$k</td>")
            print(b, "<td style=\"text-align: center; color: gray\">=</td>")
            print(b, "<td style=\"text-align: left;\">$(labelstring(v))</td>")
            println(b, "</tr>")
        end
        println(b, "</table>")
        String(take!(b))
...
parameterunits(c::Config) = [fieldunit(k, v) for (k, v) in parameterkeys(c)]
